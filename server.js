...
// A tiny status page so you can check the server is alive from a browser.
app.get("/health", (req, res) => res.json({ ok: true, connected: tiktokConnected }));

// ----------------------------------------------------------------------------
// 2. Country data — capital + coordinates, used to compute distance/direction
//    for every guess. This is a compact built-in list (no external file
//    needed). Shapes themselves are fetched by the browser from a public
//    map CDN, keyed by these same ISO codes.
// ----------------------------------------------------------------------------
...
function findCountryByGuess(text) {
  ...
  // loose match: guess text contains, or is contained by, a known name
...
// ----------------------------------------------------------------------------
// 3. Geo math — distance (km) + compass direction between two guesses
// ----------------------------------------------------------------------------
...
// Max distance on Earth (antipodal, ~20015 km) — used to compute % proximity
const MAX_KM = 20015;
...
let game = {
  target: null,          // current country object
  guesses: [],           // [{ name, distanceKm, direction, percent, byWhom, correct }]
  round: 0,
  status: "idle",        // idle | playing | won | revealed
  difficulty: "all",     // all | easy (well-known countries only)
  startedAt: null,
};

let leaderboard = new Map(); // username -> points
const guessedThisRound = new Set(); // usernames who already guessed this round (1 guess each)

function pickTarget() {
  const pool =
    game.difficulty === "easy" ? COUNTRIES.filter((c) => c.easy) : COUNTRIES;
...
function broadcastState() {
  io.emit("state", {
    ...
    // only reveal target's shape id + iso (not the name!) while playing
...
function handleGuess(rawText, username, { isHost = false } = {}) {
  ...
  if (!isHost) {
    if (guessedThisRound.has(username)) return; // 1 guess per viewer per round
  }
  const guessed = findCountryByGuess(rawText);
  if (!guessed) return; // not a recognizable country name, ignore silently
  if (!isHost) guessedThisRound.add(username);

  const dist = haversineKm(
    guessed.lat, guessed.lon,
    game.target.lat, game.target.lon
  );
  ...
  if (correct) {
    ...
    // small participation point for a valid attempt
...
// ----------------------------------------------------------------------------
// 5. Host controls (sent from the on-screen host panel)
// ----------------------------------------------------------------------------
io.on("connection", (socket) => {
  socket.emit("state", null); // trigger client to request full state
  broadcastState();
  ...
  socket.on("host:guess", (text) => handleGuess(text, "Host", { isHost: true }));
});
...
function connectToTikTok() {
  ...
  tiktokConn.on("chat", (data) => {
    ...
    io.emit("chat", { username, comment }); // show in the live comment feed
    handleGuess(comment, username);
  });
